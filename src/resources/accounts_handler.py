...
class AccountsHandler(BaseHandler):
    ...
    @auth.login_required
    def get(self):
        accounts = self.repository.user_list_accounts(g.user.user_id)
        if not accounts:
    ...
    def post(self):
        ...
        if self.repository.account_is_a_duplicate(g.user.user_id, name):
            abort(400, error='Account already exists')
        account_id = self.repository.account_create(type_id, name, g.user.user_id, currency_id, balance)
